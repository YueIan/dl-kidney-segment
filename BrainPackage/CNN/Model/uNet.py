...
class UNet(nn.Module):
    def __init__(self,colordim =1, class_type = 2, is_need_sigmoid = True):
        super(UNet, self).__init__()
        self.conv1_1 = nn.Conv2d(colordim, 64, 3, padding = 1)  # input of (n,n,1), output of (n,n,64)
        ...
        self.conv9_3 = nn.Conv2d(64, class_type, 1)
        self.bn9 = nn.BatchNorm2d(class_type)
    ...
    def forward(self, x1):
        ...
        # xup =self.conv9_3(F.relu(self.conv9_2(F.relu(self.conv9_1(xup)))))
        # return xup
        xup = self.bn9(xup)
        if self.is_need_sigmoid == True:
            xup = nn.functional.sigmoid(xup)         
        return xup
